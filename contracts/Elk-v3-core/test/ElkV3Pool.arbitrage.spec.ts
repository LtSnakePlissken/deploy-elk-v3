...
import { ElkV3PoolSwapTest } from '../typechain/ElkV3PoolSwapTest'
...
describe('ElkV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('ElkV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as ElkV3PoolSwapTest
