...
import { TestElkV3SwapPay } from '../typechain/TestElkV3SwapPay'
...
import { TestElkV3ReentrantCallee } from '../typechain/TestElkV3ReentrantCallee'
...
describe('ElkV3Pool', () => {
  ...
  let factory: ElkV3Factory
  let pool: MockTimeElkV3Pool

  let swapTarget: TestElkV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeElkV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Elk/Elk-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestElkV3ReentrantCallee')
      ).deploy()) as TestElkV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestElkV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestElkV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestElkV3SwapPay
